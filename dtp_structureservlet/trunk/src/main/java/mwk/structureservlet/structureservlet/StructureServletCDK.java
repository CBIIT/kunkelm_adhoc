...
//      }
  ...
  protected final void renderNsc(Integer nsc, Integer structureDim, HttpServletResponse response) {
    ...
    try {
      ...
      if (sVO.getSmiles() == null || sVO.getSmiles().isEmpty()) {
        ...
        sb.insert(0, "No smiles for NSC" + nsc);
        byteArray = getTextImage("No smiles for NSC" + nsc);
        ...
        String title = "NSC" + nsc + "xxxFAKEMW:123xxxFAKENAME:DrugNamexxxFAKECAS:12-34-56";
        String querySmiles = "";

        try {
          byteArray = renderImage(title, smiles, querySmiles, structureDim);
        } catch (Exception e) {
          byteArray = getTextImage("Unable to render smiles: " + smiles + " for nsc: " + nsc);
        }
      }
      ...
      System.out.println("Unexpected exception in renderNsc in StructureServlet");
      e.printStackTrace();
    }
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if (request.getParameterValues("nscArray") != null) {
      nscIntList = new ArrayList<Integer>();
      String[] intStrArr = request.getParameterValues("nscArray");
      for (String nscStr : intStrArr) {
        nscIntList.add(Integer.valueOf(nscStr));
      }
      for (Integer i : nscIntList) {
        System.out.println("nscIntList: " + i);
      }
    }
