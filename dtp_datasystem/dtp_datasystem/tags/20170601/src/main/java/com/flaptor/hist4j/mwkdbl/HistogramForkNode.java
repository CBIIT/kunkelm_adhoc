...
/**
 * The HistogramForkNode splits the data range in two at a given value, pointing
 * to two subtrees, one for values smaller than the split value, and one for
 * values larger than the split value. It implements the recursive calls
 * necesary to obtain the data from the tree structure.
 *
 * @author Jorge Handl
 */
public class HistogramForkNode extends HistogramNode {
  ...
  /**
   * Adds a value to the histogram by recursively adding the value to either
   * subtree, depending on the split value.
   *
   ...
   *
   * @param accumCount accumCount[0] the running cumulative count. accumCount[1]
   * the target cumulative count.
  ...
  /**
   * Build the table representing the histogram data adding the data from each
   * subtree.
   */
