...
select pws.nsc as id, 'NSC'||pws.nsc as name, pws.nsc as nsc_cmpd_id, 's'::varchar(4) as prefix, pws.nsc, src.conf, src.cas, src.distribution_code as distribution, pws.nsc as cmpd_parent_fragment_fk, bio.id as cmpd_bio_assay_fk, inv.id as cmpd_inventory_fk, matched_salt as salt_name, salt_smiles 
from parent_with_salt pws 
left outer join cmpd_bio_assay bio on pws.nsc = bio.id
left outer join cmpd_inventory inv on pws.nsc = inv.id
...
insert into nsc_cmpd(id, name, nsc_cmpd_id, prefix, nsc, conf, cas, distribution, cmpd_parent_fragment_fk, cmpd_bio_assay_fk, cmpd_inventory_fk, salt_name, salt_smiles)
select id, name, nsc_cmpd_id, prefix, nsc, conf, cas, distribution, cmpd_parent_fragment_fk, cmpd_bio_assay_fk, cmpd_inventory_fk, salt_name, salt_smiles 
from temp;
...
-- ALL OF THIS filters, effectively, against only those compounds
-- that were in parent_with_salt since that is the basis
-- for populating nsc_cmpd
