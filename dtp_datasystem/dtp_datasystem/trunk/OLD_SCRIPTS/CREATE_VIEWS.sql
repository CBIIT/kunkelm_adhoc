...
drop view if exists formatted_strings_view;

--
--formatted_strings_view string_agg of one-to-many props
--

-- MWK 23July2014 this checks out OK with postgres explain

create view formatted_strings_view
as
select
nsc.id, 
nsc.nsc,
...
from prod_src src left outer join formatted_strings_view props on (src.nsc = props.nsc)
left outer join parent_fragment_view parent on (src.nsc = parent.nsc)
--from parent_fragment_view parent left outer join formatted_strings_view props on (parent.nsc = props.nsc)
left outer join all_fragments_view frags on (src.nsc = frags.nsc);
            
alter view formatted_strings_view owner to mwkunkel;
