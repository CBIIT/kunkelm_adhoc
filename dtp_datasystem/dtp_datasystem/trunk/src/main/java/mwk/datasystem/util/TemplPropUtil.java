...
public class TemplPropUtil<T> {
    ...
    private TreeMap<String, ArrayList<StringPair>> collatedProps;
    private TreeMap<String, ArrayList<StringPair>> dupProps;
    private TreeMap<String, ArrayList<StringPair>> successDupProps;

    // the final product(s)
    private TreeMap<String, StringPair> uniqProps;
    ...
    public TemplPropUtil(T tIn, ArrayList<String> ignListIn) {
        ...
        collatedProps = new TreeMap<String, ArrayList<StringPair>>();
        dupProps = new TreeMap<String, ArrayList<StringPair>>();
        successDupProps = new TreeMap<String, ArrayList<StringPair>>();
        uniqProps = new TreeMap<String, StringPair>();
        ...
        for (StringTriplet trip : rawProps) {
            ...
            if (this.ignList == null || this.ignList.size() == 0) {
                // ok remains true;
            } else if (CollectionUtils.containsAny(strList, this.ignList)) {
        ...
        for (String curDupKey : dupProps.keySet()) {
            ...
            for (int tryCnt = 1; tryCnt < 10; tryCnt++) {
                ...
                TreeMap<String, StringPair> tryProps = new TreeMap<String, StringPair>();
    ...
    public void printUniqProps() {
        for (String k : this.uniqProps.keySet()) {
            System.out.printf("%30s%45s%30s%n", k, this.uniqProps.get(k).fullName, this.uniqProps.get(k).dataType);
        }
    }
