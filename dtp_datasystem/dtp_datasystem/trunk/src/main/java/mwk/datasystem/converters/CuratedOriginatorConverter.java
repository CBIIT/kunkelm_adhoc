 ...
 */
@SessionScoped
@ManagedBean(name = "curatedOriginatorConverter")
@FacesConverter("curatedOriginatorConverter")
public class CuratedOriginatorConverter implements Converter, Serializable {
...
// reach-through to curatedNscController
    ...
    public Object getAsObject(FacesContext facesContext, UIComponent component, String submittedValue) throws ConverterException {
        ...
        if (submittedValue != null && submittedValue.trim().length() > 0) {
            try {
                if (curatedNscController.getKnownOriginatorsMap().containsKey(submittedValue)) {
                    rtn = curatedNscController.getKnownOriginatorsMap().get(submittedValue);
                } else {
                ...
                throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Conversion Error", "Not a valid CuratedOriginatorVO."));
            }
        }
        return rtn;
    }
    ...
    public String getAsString(FacesContext context, UIComponent component, Object modelValue) throws ConverterException {
        String rtn = null;
        if (modelValue != null && modelValue instanceof CuratedOriginatorVO) {
            rtn = ((CuratedOriginatorVO) modelValue).getValue();
        } else {
            FacesMessage errMsg = new FacesMessage(CONVERSION_ERROR_MESSAGE_ID);
            FacesContext.getCurrentInstance().addMessage(null, errMsg);
            throw new ConverterException(errMsg.getSummary());
