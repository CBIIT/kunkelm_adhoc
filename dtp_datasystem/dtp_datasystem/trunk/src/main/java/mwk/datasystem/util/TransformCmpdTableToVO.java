...
public class TransformCmpdTableToVO {
    ...
    private static CmpdFragmentPChemVO translateCmpdFragmentPChem(CmpdTable entityIn) {
        ...
        // the ONLY fragment from CmpdTable is the parentFragment
        ...
        try {
            ...
            if (entityIn != null) {
                ...
                rtn.setMolecularWeight(entityIn.getParentMolecularWeight());
                rtn.setMolecularFormula(entityIn.getParentMolecularFormula());
    ...
    public static CmpdVO translateCmpd(CmpdTable entityIn) {
        ...
        try {
            ...
            cmpdVO.setFormulaMolecularFormula(entityIn.getFormulaMolecularFormula());
            cmpdVO.setFormulaWeight(entityIn.getFormulaWeight());

            cmpdVO.setParentFragment(translateCmpdFragment(entityIn));
            cmpdVO.setParentMolecularWeight(entityIn.getParentMolecularWeight());
            cmpdVO.setParentMolecularFormula(entityIn.getParentMolecularFormula());

            if (entityIn.getSaltStoichiometry() != null && entityIn.getSaltStoichiometry().doubleValue() > 0) {
                CmpdFragmentVO cfVO = new CmpdFragmentVO();
                cfVO.setSaltMf(entityIn.getSaltMf());
                cfVO.setSaltMw(entityIn.getSaltMw());
                cfVO.setSaltName(entityIn.getSaltName());
