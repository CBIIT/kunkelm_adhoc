...
/**
 * This class implements a histogram that adapts to an unknown data
 * distribution. It keeps a more or less constant resolution throughout the data
 * range by increasing the resolution where the data is more dense. For example,
 * if the data has such such a distribution that most of the values lie in the
 * 0-5 range and only a few are in the 5-10 range, the histogram would adapt and
 * assign more counting buckets to the 0-5 range and less to the 5-10 range.
 * This implementation provides a method to obtain the accumulative density
 * function for a given data point, and a method to obtain the data point that
 * splits the data set at a given percentile.
 *
 * @author Jorge Handl
 */
public class AdaptiveHistogram implements Serializable {
  ...
  // MWK added to try to overcome the problems with 
  ...
  /**
   * This method is used by the internal data structure of the histogram to get
   * the limit of data points that should be counted at one bucket.
   *
  ...
  /**
   * Return a table representing the data in this histogram. Each element is a
   * table cell containing the range limit values and the count for that range.
   */
