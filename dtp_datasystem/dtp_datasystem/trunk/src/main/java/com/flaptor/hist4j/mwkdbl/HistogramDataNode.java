...
/**
 * The HistogramDataNode stores the histogram data for a range of values. It
 * knows the minimum and maximum values for which it counts the number of
 * instances. When the count exceeds the allowed limit it splits itself in two,
 * increasing the histogram resolution for this range.
 *
 * @author Jorge Handl
 */
public class HistogramDataNode extends HistogramNode {
  ...
  /**
   * Adds a value to the data node.<p> If the value falls inside of the nodes'
   * range and the count does not exceed the imposed limit, it simply increments
   * the count.<br> If the value falls outside of the nodes' range, it expands
   * the range.<br> If the count exceeds the limit, it splits in two assuming
   * uniform distribution inside the node.<br> If the value falls outside of the
   * nodes' range AND the count exceeds the limit, it creates a new node for
   * that value.
   *
   * @param root a reference to the adaptive histogram instance that uses this
   * structure.
   * @param value the value for which the count is to be incremented.
   * @return A reference to itself if no structural change happened, or a
   * reference to the new fork node if this node was split.
   */
  ...
  /**
   * Returns the number of data points stored in the same bucket as a given
   * value.
   *
  ...
  /**
   * Returns the data point where the running cumulative count reaches the
   * target cumulative count. It uses linear interpolation over the range of the
   * node to get a better estimate of the true value.
   *
   * @param accumCount an array containing:<br> - accumCount[0] the running
   * cumulative count. <br> - accumCount[1] the target cumulative count.
   * @return the data point where the running cumulative count reaches the
   ...
   *
   * @param valueConversion a class that defines a function to convert the
   * value.
   */
  ...
  /**
   * Prints this nodes' data with a margin depending on the level of the node in
   * the tree.
   *
  ...
  /**
   * Build the table representing the histogram data adding this node's cell to
   * it.
   */
