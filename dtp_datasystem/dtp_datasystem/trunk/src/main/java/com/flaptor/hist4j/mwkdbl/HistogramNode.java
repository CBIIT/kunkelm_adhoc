 ...
 * provides a common inteface for both the data and the fork nodes. This is an
 * abstract class, and it is instantiated as either a fork node or a data node.
 *
 * @author Jorge Handl
 */
public abstract class HistogramNode implements Serializable {
   ...
   */
  public abstract HistogramNode addValue(AdaptiveHistogram root, double value);

  /**
   * Abstract method for getting the number of values stored in the same bucket
   * as a reference value
   */
  public abstract long getCount(double value);

  /**
   * Abstract method for getting the cumulative density function for a given
   * value
   */
  public abstract long getAccumCount(double value);

  /**
   * Abstract method for getting the value for which the cumulative density
   * function reaches the desired value
   */
  public abstract Double getValueForAccumCount(long[] accumCount);

  /**
   * Abstract method for applying a convertion function to the values stored in
   * the histogram
   */
