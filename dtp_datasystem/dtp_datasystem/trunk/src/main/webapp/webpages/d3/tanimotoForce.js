...
// clear structures
  ...
  filtNodes.forEach(function(nod) {
    ...
    if (selTargets.indexOf(nod.t) > -1 || selDrugNames.indexOf(nod.name) > -1 || selNscs.indexOf(nod.nsc) > -1) {
...
// link lbl to par 
  ...
  var circle = node.append("svg:path").attr("d", d3.svg.symbol().type(function(d) {
    ...
    return selTargets.indexOf(d.t) > -1 || selDrugNames.indexOf(d.name) > -1 || selNscs.indexOf(d.nsc) > -1 ? 300 : 100; //d.symbolSize * 3 : d.symbolSize;
  })).attr("fill", function(d) {
    return selTargets.indexOf(d.t) > -1 || selDrugNames.indexOf(d.name) > -1 || selNscs.indexOf(d.nsc) > -1 ? 'red' : d.symbolColor;
  }).attr("stroke", "black");
...
// else if nsc with drug name, or moltId then do text
      ...
      if (d.node.nsc && d.node.smiles === 'ALWAYS FAILS') {
        ...
        d3.select(this).append("svg:text").text(function(d, i) {
          return d.node.drugName ? d.node.nsc + ' ' + d.node.drugName : d.node.nsc;
        }).style("fill", "#555");
      } else if (d.node.moltId) {
        d3.select(this).append("svg:text").text(function(d, i) {
          return d.node.genecare ? d.node.moltId + ' ' + d.node.genecard : d.node.moltId;
        }).style("fill", "#555");
      } else {
