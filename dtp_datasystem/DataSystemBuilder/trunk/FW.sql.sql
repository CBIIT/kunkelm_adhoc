...
-- comparison of fw by rdkit_mol_frag with the prod_mw (fw)

drop table if exists rdk_fw;
create table rdk_fw
as
select nsc, sum(mw) as rdk_fw, count(mw) as count_frag
from rdkit_mol_frag
group by nsc;

drop table if exists compare_fw;
create table compare_fw 
as
select rdk.nsc, rdk.rdk_fw, count_frag, mw, @((rdk_fw - mw) / rdk_fw) as pct_diff
from rdk_fw rdk, prod_src prod
where rdk.nsc = prod.nsc;

select count(*) from compare_fw;
select count(*) as less_than_1_pct from compare_fw where pct_diff < 0.01;
select count(*) as less_than_2_pct from compare_fw where pct_diff < 0.02;
select count(*) as less_than_5_pct from compare_fw where pct_diff < 0.05;

select count(*) from compare_fw where count_frag > 1;
select count(*) as less_than_1_pct from compare_fw where pct_diff < 0.01 and count_frag > 1;
select count(*) as less_than_2_pct from compare_fw where pct_diff < 0.02 and count_frag > 1;
select count(*) as less_than_5_pct from compare_fw where pct_diff < 0.05 and count_frag > 1;
...
create table plp_related(ref_nsc int, rel_nsc int, relation varchar(1024));
\copy plp_related from /home/mwkunkel/PROJECTS/CURRENT/dtp_datasystem/DataSystemBuilder/related.06Dec2013.pipelinePilot.txt csv header delimiter as E'\t' quote as '"'

update plp_related set relation = 'identical' where relation = 'IDENTICAL';
update plp_related set relation = 'identical' where relation = 'IGNORE_STEREO';
update plp_related set relation = 'salt' where relation = 'SALT';

create index related_ref_nsc on related(ref_nsc);
create index related_rel_nsc on related(rel_nsc);

create index plp_related_ref_nsc on plp_related(ref_nsc);
create index plp_related_rel_nsc on plp_related(rel_nsc);

select count(distinct foo.ref_nsc) 
from
(
select ref_nsc from related except select nsc from parent_with_salt
) as foo;

-- what relations come from one but not the other?

select count(*) from plp_related;
select count(*) from related;

drop table if exists temp;

create table temp as 
select ref_nsc, rel_nsc, relation 
--from plp_related
from related
except select ref_nsc, rel_nsc, relation
--from related;
from plp_related;

select count(*) from temp;

-- bring in mf from prod_src

drop table if exists temp2;

create table temp2
as
select p.ref_nsc, p.rel_nsc, p.relation, prod.mf as ref_mf, prod2.mf as rel_mf
from plp_related p, temp t, prod_src prod, prod_src prod2
where p.ref_nsc = t.ref_nsc
and p.rel_nsc = t.rel_nsc
and p.ref_nsc = prod.nsc
and p.rel_nsc = prod2.nsc;

-- bring in parent from parent_with_salt

drop table if exists temp2;

create table temp2
as
select p.ref_nsc, p.rel_nsc, p.relation, pws.inchi as ref_inchi, pws2.inchi as rel_inchi
from plp_related p, temp t, parent_with_salt pws, parent_with_salt pws2
where p.ref_nsc = t.ref_nsc
and p.rel_nsc = t.rel_nsc
and p.ref_nsc = pws.nsc
and p.rel_nsc = pws2.nsc;
